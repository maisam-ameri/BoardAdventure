...
### Mock Data Prototype
The initial stable version of this project relies on **Mock Data**. 
If you need to revisit or test the implementation where local mock data drives the game logic, please checkout the dedicated tag:
...
The project supports a local network configuration file for multiplayer setup.

- `Secrets/network_config_example.json`  Serves as the template file. Please set your AppID here when
- `Secrets/network_config.json` is the real runtime config file and must remain local.

### NetworkConfigLoader

`NetworkConfigLoader` is the utility class responsible for reading the network config file during startup.  
It expects the config file name to be:
```csharp
...
- ✅ Network config handled  
- ✅ Network lobby implemented
